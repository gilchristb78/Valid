...
public class MoveColumn extends Move {
    ...
    public boolean valid(Solitaire game) {
        return (true && (ConstraintHelper.ifCompute(destination.empty(), () -> ((true && movingCards.descending()) && movingCards.alternatingColors()), () -> (destination.peek().getRank() == (movingCards.peek(0).getRank() % 13) + 1 && movingCards.peek(0).oppositeColor(destination.peek()))) && ConstraintHelper.sufficientFree(movingCards, source, destination, ConstraintHelper.tableau(game), ConstraintHelper.reserve(game))));
    }
}
