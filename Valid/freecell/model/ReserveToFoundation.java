 ...
 */
public class ReserveToFoundation extends Move {

    protected Stack destination;

    protected Stack source;

    public ReserveToFoundation(Stack from, Stack to) {
        super();
        this.source = from;
        this.destination = to;
    }
    ...
    public ReserveToFoundation(Stack from, Card card, Stack to) {
        this(from, to);
    ...
    public boolean valid(Solitaire game) {
        return (true && ConstraintHelper.ifCompute(destination.empty(), () -> movingCard.getRank() == Card.ACE, () -> (movingCard.getRank() == (destination.peek().getRank() % 13) + 1 && movingCard.getSuit() == destination.peek().getSuit())));
    }
}
