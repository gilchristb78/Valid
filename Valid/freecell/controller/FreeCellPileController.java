package org.combinators.solitaire.freecell.controller;

import org.combinators.solitaire.freecell.*;
...
public class FreeCellPileController extends SolitaireReleasedAdapter {
     ...
     */
    protected FreeCellPileView src;

    public FreeCellPileController(FreeCell theGame, FreeCellPileView src) {
        super(theGame);
    ...
    public void mousePressed(MouseEvent me) {
        ...
        me_ignore = false;
        // Return in the case that the widget clicked on is empty
        FreeCellPile srcElement = (FreeCellPile) src.getModelElement();
    ...
    public void mouseReleased(MouseEvent me) {
        ...
        if (w instanceof ColumnView) {
            ...
            try {
                ...
                FreeCellPile toElement = (FreeCellPile) src.getModelElement();
                // Identify the source
                Column sourceEntity = (Column) sourceWidget.getModelElement();
                // this is the actual move
                Move m = new TableauToReserve(sourceEntity, movingElement, toElement);
                if (m.valid(theGame)) {
                    m.doMove(theGame);
                    theGame.pushMove(m);
                } else {
        ...
        if (w instanceof CardView) {
            ...
            try {
                ...
                FreeCellPile toElement = (FreeCellPile) src.getModelElement();
                // Identify the source
                FreeCellPile sourceEntity = (FreeCellPile) sourceWidget.getModelElement();
                // this is the actual move
                Move m = new ReserveToReserve(sourceEntity, movingElement, toElement);
                if (m.valid(theGame)) {
                    m.doMove(theGame);
                    theGame.pushMove(m);
                } else {
        ...
        ((org.combinators.solitaire.freecell.FreeCell) theGame).tryAutoMoves();
