...
public class AcesUpPileController extends SolitaireReleasedAdapter {
     ...
     */
    protected AcesUpPileView src;

    public AcesUpPileController(Archway theGame, AcesUpPileView src) {
        super(theGame);
    ...
    public void mouseReleased(MouseEvent me) {
        ...
        if (w instanceof CardView) {
            ...
            try {
                ...
                AcesUpPile toElement = (AcesUpPile) src.getModelElement();
                // Identify the source
                Column sourceEntity = (Column) sourceWidget.getModelElement();
                // this is the actual move
                Move m = new TableauToFoundation(sourceEntity, movingElement, toElement);
                if (m.valid(theGame)) {
                    m.doMove(theGame);
                    theGame.pushMove(m);
                } else {
        ...
        if (w instanceof CardView) {
            ...
            try {
                ...
                // this is the actual move
                Move m = new ReserveToFoundation(sourceEntity, movingElement, toElement);
                if (m.valid(theGame)) {
                    m.doMove(theGame);
                    theGame.pushMove(m);
                } else {
