package org.combinators.solitaire.archway.controller;

import org.combinators.solitaire.archway.*;
...
public class KingsDownPileController extends SolitaireReleasedAdapter {

    protected Archway theGame;

    /**
     * The View being controlled
     */
    protected KingsDownPileView src;

    public KingsDownPileController(Archway theGame, KingsDownPileView src) {
        super(theGame);
    ...
    public void mouseReleased(MouseEvent me) {
        ...
        if (w instanceof CardView) {
            ...
            try {
                ...
                KingsDownPile toElement = (KingsDownPile) src.getModelElement();
                // Identify the source
                Column sourceEntity = (Column) sourceWidget.getModelElement();
                // this is the actual move
                Move m = new TableauToKingsFoundation(sourceEntity, movingElement, toElement);
                if (m.valid(theGame)) {
                    m.doMove(theGame);
                    theGame.pushMove(m);
                } else {
        ...
        if (w instanceof CardView) {
            ...
            try {
                ...
                // this is the actual move
                Move m = new ReserveToKingsFoundation(sourceEntity, movingElement, toElement);
                if (m.valid(theGame)) {
                    m.doMove(theGame);
                    theGame.pushMove(m);
                } else {
