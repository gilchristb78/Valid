 ...
 */
public class ReserveToKingsFoundation extends Move {

    protected Stack destination;

    protected Stack source;

    public ReserveToKingsFoundation(Stack from, Stack to) {
        super();
        this.source = from;
        this.destination = to;
    }
    ...
    public ReserveToKingsFoundation(Stack from, Card card, Stack to) {
        this(from, to);
    ...
    public boolean valid(Solitaire game) {
        return (true && (destination.peek().getRank() == (movingCard.getRank() % 13) + 1 && movingCard.getSuit() == destination.peek().getSuit()));
    }
}
