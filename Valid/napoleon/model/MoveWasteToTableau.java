package org.combinators.solitaire.napoleon.model;
 ...
 */
public class MoveWasteToTableau extends Move {

    protected Stack destination;

    protected Stack source;

    public MoveWasteToTableau(Stack from, Stack to) {
        super();
        this.source = from;
        this.destination = to;
    }
    ...
    public MoveWasteToTableau(Stack from, Card card, Stack to) {
        this(from, to);
    ...
    public boolean valid(Solitaire game) {
        return (true && (destination.empty() || (destination.peek().getRank() == (movingCard.getRank() % 13) + 1 && destination.peek().getSuit() == movingCard.getSuit())));
    }
}
