 ...
 */
public class Tableau extends Move {

    protected Stack destination;

    protected Stack source;

    public Tableau(Stack from, Stack to) {
        super();
        this.source = from;
        this.destination = to;
    }
    ...
    public Tableau(Stack from, Card card, Stack to) {
        this(from, to);
    ...
    public boolean valid(Solitaire game) {
        return (true && (destination.empty() || (!(movingCard.getSuit() == destination.peek().getSuit()) && destination.peek().getRank() == movingCard.getRank() + 1)));
    }
}
