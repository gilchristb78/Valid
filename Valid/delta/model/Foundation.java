package org.combinators.solitaire.delta.model;
 ...
 */
public class Foundation extends Move {

    protected Stack destination;

    protected Stack source;

    public Foundation(Stack from, Stack to) {
        super();
        this.source = from;
        this.destination = to;
    }
    ...
    public Foundation(Stack from, Card card, Stack to) {
        this(from, to);
    ...
    public boolean valid(Solitaire game) {
        return (true && ConstraintHelper.ifCompute(destination.empty(), () -> movingCard.getRank() == Card.KING, () -> (movingCard.getSuit() == destination.peek().getSuit() && destination.peek().getRank() == movingCard.getRank() + 1)));
    }
}
