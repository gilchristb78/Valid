package org.combinators.solitaire.shared.javatest
...
import org.combinators.solitaire.shared.{Base, SemanticTypes, SolitaireDomain}
...
trait TestLogic extends SemanticTypes with Base {
  override def init[G <: SolitaireDomain](gamma : ReflectedRepository[G], s:Solitaire) : ReflectedRepository[G] = {
    var updated = super.init(gamma, s)

    // Handle Dealing cards.
    updated = updated
      .addCombinator(new ProcessUnitTests(s))

    updated
  }

  /**
    * Allow for synthesizing unit test cases using JUnit 4
    */
  class ProcessUnitTests(s:Solitaire) {
