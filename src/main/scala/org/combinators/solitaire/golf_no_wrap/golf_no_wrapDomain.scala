package org.combinators.solitaire.golf_no_wrap
...
class golf_no_wrapDomain(override val solitaire:Solitaire) extends SolitaireDomain(solitaire) with GameTemplate with Controller {
  ...
  object golf_no_wrapCodeGenerator {
    val generators:CodeGeneratorRegistry[Expression] = CodeGeneratorRegistry.merge[Expression](
      ...
      CodeGeneratorRegistry[Expression, ToLeftOf] {
        case (registry:CodeGeneratorRegistry[Expression], c:ToLeftOf) =>
          ...
          Java(s"""((org.combinators.solitaire.golf_no_wrap.Golf_no_wrap)game).toLeftOf($destination, $src)""").expression()
      ...
      CodeGeneratorRegistry[Expression, AllSameRank] {
        case (registry:CodeGeneratorRegistry[Expression], all:AllSameRank) =>
          ...
          Java(s"""((org.combinators.solitaire.golf_no_wrap.Golf_no_wrap)game).allSameRank($inner)""").expression()

      }
    ).merge(constraintCodeGenerators.generators)
  }

  @combinator object golf_no_wrapGenerator {
    def apply: CodeGeneratorRegistry[Expression] = {
      print ("In ggen")
      golf_no_wrapCodeGenerator.generators
    }
  ...
  @combinator object HelperMethodsGolf_no_wrap {
