...
package object spider {
  ...
  for (colNum <- 0 to 3) {
    dealSeq = dealSeq :+ DealStep(ElementTarget(Tableau, colNum), Payload(faceUp = false, numCards = 5)) //SolitaireContainerTypes.Tableau -> Tableau
  }
  ...
  val spider:Solitaire = {
    ...
    Solitaire( name="Spider",
      ...
      deal = dealSeq,//Seq(DealStep(ContainerTarget(Tableau))),
