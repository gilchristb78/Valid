package org.combinators.solitaire.milligancell;
...
import org.combinators.solitaire.milligancell.controller.*;
import org.combinators.solitaire.milligancell.model.*;
 ...
 */
public class MilliganCell extends Solitaire {
    ...
    public void initialize() {
        ...
        for (int j = 0; j < 4; j++) {
            reserve[j] = new FreeCellElement(reservePrefix + (j + 1));
            addModelElement(reserve[j]);
            reserveView[j] = new FreeCellElementView(reserve[j]);
        }
        tableauView[0].setBounds(100, 200, 73, 1261);
        addViewWidget(tableauView[0]);
        tableauView[1].setBounds(188, 200, 73, 1261);
        addViewWidget(tableauView[1]);
        tableauView[2].setBounds(276, 200, 73, 1261);
        addViewWidget(tableauView[2]);
        tableauView[3].setBounds(364, 200, 73, 1261);
        addViewWidget(tableauView[3]);
        tableauView[4].setBounds(452, 200, 73, 1261);
        addViewWidget(tableauView[4]);
        tableauView[5].setBounds(540, 200, 73, 1261);
        addViewWidget(tableauView[5]);
        tableauView[6].setBounds(628, 200, 73, 1261);
        addViewWidget(tableauView[6]);
        tableauView[7].setBounds(716, 200, 73, 1261);
        ...
        reserveView[0].setBounds(15, 200, 73, 97);
        addViewWidget(reserveView[0]);
        reserveView[1].setBounds(15, 312, 73, 97);
        addViewWidget(reserveView[1]);
        reserveView[2].setBounds(15, 424, 73, 97);
        addViewWidget(reserveView[2]);
        reserveView[3].setBounds(15, 536, 73, 97);
        ...
        for (int j = 0; j < 4; j++) {
            ...
            reserveView[j].setMouseAdapter(new FreeCellElementController(this, reserveView[j]));
        }
        for (int i = 0; i < 4; i++) {
    ...
    public static void main(String[] args) {
        final GameWindow gw = Main.generateWindow(new MilliganCell(), Deck.OrderBySuit);
    ...
    public FreeCellElement[] reserve = new FreeCellElement[4];

    public static final String reservePrefix = "reserve";

    public FreeCellElementView[] reserveView = new FreeCellElementView[4];
