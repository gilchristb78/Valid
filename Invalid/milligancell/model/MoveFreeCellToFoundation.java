package org.combinators.solitaire.milligancell.model;
 ...
 */
public class MoveFreeCellToFoundation extends Move {

    protected Stack destination;

    protected Stack source;

    public MoveFreeCellToFoundation(Stack from, Stack to) {
        super();
        this.source = from;
        this.destination = to;
    }
    ...
    public MoveFreeCellToFoundation(Stack from, Card card, Stack to) {
        this(from, to);
    ...
    public boolean valid(Solitaire game) {
        return (true && ((destination.empty() && movingCard.getRank() == Card.ACE) || (movingCard.getRank() == (destination.peek().getRank() % 13) + 1 && movingCard.getSuit() == destination.peek().getSuit())));
    }
}
