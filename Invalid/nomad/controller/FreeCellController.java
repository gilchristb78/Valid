package org.combinators.solitaire.nomad.controller;
...
public class FreeCellController extends SolitaireReleasedAdapter {

    protected Nomad theGame;

    /**
     * The View being controlled
     */
    protected FreeCellView src;

    public FreeCellController(Nomad theGame, FreeCellView src) {
        super(theGame);
    ...
    public void mousePressed(MouseEvent me) {
        ...
        me_ignore = false;
        // Return in the case that the widget clicked on is empty
        FreeCell srcElement = (FreeCell) src.getModelElement();
    ...
    public void mouseReleased(MouseEvent me) {
        ...
        if (w instanceof ColumnView) {
            ...
            try {
                ...
                FreeCell toElement = (FreeCell) src.getModelElement();
                // Identify the source
                BuildablePile sourceEntity = (BuildablePile) sourceWidget.getModelElement();
                // this is the actual move
                Move m = new TableauToFreeCell(sourceEntity, movingElement, toElement);
                if (m.valid(theGame)) {
                    m.doMove(theGame);
                    theGame.pushMove(m);
                } else {
        ...
        if (w instanceof CardView) {
            ...
            try {
                ...
                // this is the actual move
                Move m = new MoveFoundationToFreeCell(sourceEntity, movingElement, toElement);
                if (m.valid(theGame)) {
                    m.doMove(theGame);
                    theGame.pushMove(m);
                } else {
