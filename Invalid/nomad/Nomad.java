...
import org.combinators.solitaire.nomad.controller.*;
 ...
 */
public class Nomad extends Solitaire {
    ...
    public void initialize() {
        ...
        for (int j = 0; j < 1; j++) {
            reserve[j] = new FreeCell(reservePrefix + (j + 1));
            addModelElement(reserve[j]);
            reserveView[j] = new FreeCellView(reserve[j]);
        }
        ...
        for (int j = 0; j < 1; j++) {
            ...
            reserveView[j].setMouseAdapter(new FreeCellController(this, reserveView[j]));
        }
        for (int i = 0; i < 4; i++) {
    ...
    public static void main(String[] args) {
        final GameWindow gw = Main.generateWindow(new Nomad(), Deck.OrderBySuit);
    ...
    public FreeCell[] reserve = new FreeCell[1];

    public static final String reservePrefix = "reserve";

    public FreeCellView[] reserveView = new FreeCellView[1];
